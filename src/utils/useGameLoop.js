...
export function useGameLoop({
  ...
  // Authoritative vertical physics state exposed to Player.js
  const [playerVelocityY, setPlayerVelocityY] =
    useState(0);

  // Number of physics frames spent airborne
  const [airborneFrame, setAirborneFrame] =
    useState(0);
  ...
  // Animation state is driven by the same physics loop
  // that owns playerVelocityYRef/isGroundedRef.
  const airborneAnimationFrameRef =
    useRef(0);

  const wasGroundedForAnimationRef =
    useRef(false);
  ...
  /* Counts accepted taps independently of the physics frame. */
  const jumpTapCountRef =
    useRef(0);
  ...
  useEffect(() => {
    /*
     * Base game: one normal jump followed by one BIG
     * power jump when the player double-taps.
     *
     * Extra Jump upgrade: allows one additional jump,
     * making three total jumps in the air sequence.
     */
    maxJumpsRef.current =
      1 +
      ((upgradeLevels.extra_jump || 0) > 0
        ? 2
        : 0);
  ...
  const startGame = useCallback(() => {
    ...
    airborneAnimationFrameRef.current =
      0;

    wasGroundedForAnimationRef.current =
      true;
  ...
  const requestJump = useCallback(() => {
     ...
     * JUMP SEQUENCE
     *
     * No Extra Jump upgrade:
     *   Tap 1 -> normal jump
     *   Tap 2 -> BIG power jump
     *   Tap 3+ -> ignored
     *
     * Extra Jump upgrade purchased:
     *   Tap 1 -> normal jump
     *   Tap 2 -> BIG power jump
     *   Tap 3 -> normal third jump
     *
     * The jump is applied immediately here instead of being
     * queued for the next RAF frame. This is important because
     * two or three very fast taps can otherwise arrive before
     * the physics loop updates jumpCountRef.
     */
    ...
    if (
      ...
      /* Power jump is always jump #2. */
      jumpCountRef.current = 2;

      jumpTapCountRef.current = 2;
    ...
    if (
      ...
      jumpCountRef.current = 3;

      jumpTapCountRef.current = 3;
  ...
  useEffect(() => {
    ...
    const updateFrame = () => {
      ...
      /* Jump input is applied immediately inside requestJump. */
        ...
        for (
          ...
          if (
            ...
            if (
              ...
              /* Gravity-flip landing starts a fresh jump sequence. */
              lastTapTimestampRef.current =
                0;
              lastPowerJumpTimestampRef.current =
                0;
      ...
      // =====================
      // AUTHORITATIVE PLAYER ANIMATION STATE
      // =====================
      // Keep animation state tied to physics rather than trying
      // to infer rising/falling from rendered React Y positions.
      if (isGroundedRef.current) {
        airborneAnimationFrameRef.current =
          0;
      } else if (wasGroundedForAnimationRef.current) {
        airborneAnimationFrameRef.current =
          0;
      } else {
        airborneAnimationFrameRef.current =
          Math.min(30, airborneAnimationFrameRef.current + 1);
      }
      ...
      for (
        ...
        if (collides) {
          ...
          if (
            ...
            // A boost pad is a launch, not a grounded state.
            // Explicitly leaving grounded prevents the support
            // check on the next frame from cancelling the launch.
            isGroundedRef.current =
              false;
